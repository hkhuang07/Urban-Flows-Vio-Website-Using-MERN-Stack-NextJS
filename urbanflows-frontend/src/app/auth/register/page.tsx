...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('http://localhost:3001/api/accounts/register', {
        method: 'POST',
        headers: {
  ...
  const motto = "In the Urban Flows, Find Your Perfect Beat!"; // Thêm motto để đồng bộ

  return (
    <div className="relative flex min-h-screen items-center justify-center p-4 md:p-12">
      {/* Background with gradient and blur effect */}
      <div className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: "url('/urbanflowscity.jpg')" }}>
      ...
      <div className="relative z-10 flex w-full max-w-5xl overflow-hidden rounded-xl shadow-2xl backdrop-blur-md bg-white/10 dark:bg-gray-800/20">
        ...
        <div className="relative hidden w-1/2 md:flex items-center justify-center p-8 bg-cover bg-center" style={{ backgroundImage: "url('/52hzmelody.jpg')" }}>
        ...
        <div className="w-full md:w-1/2 p-12 bg-gradient-to-b from-[#] to-[#000005] text-white flex flex-col justify-center rounded-r-xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="relative">
              <FaEnvelope className="absolute top-1/2 left-3 transform -translate-y-1/2 text-[#5F9EA0]" />
              <input
                type="email"
