...
import { useRouter } from "next/navigation"; // Thêm useRouter để xử lý chuyển hướng
...
export default function Header() {
  ...
  useEffect(() => {
    ...
    setIsLoggedIn(!!token); // Chuyển đổi giá trị thành boolean
  }, []);
    ...
    <header className="bg-[#000624] text-white py-2 px-4 shadow-lg dark:bg-[#121212] dark:text-gray-100 transition-colors duration-300">
      <nav className="flex justify-between items-center max-w-7xl mx-auto">
        ...
        <div className="flex items-center space-x-4">
          ...
          {/* Hiển thị nút tùy thuộc vào trạng thái đăng nhập */}
          {isLoggedIn ? (
            <button
              onClick={handleLogout}
              className="px-4 py-2 flex items-center space-x-2 text-[#F0F8FF] bg-[#000010] border border-[#7FFFD4] rounded-full hover:bg-[#458B74] transition-colors duration-300"
            >
              <FaSignOutAlt />
              <span>Đăng xuất</span>
            </button>
